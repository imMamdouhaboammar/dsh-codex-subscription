...
var PendingOAuthCredentialStore = class {
	...
	async read(providerId) {
		if (providerId !== "openai-codex" && providerId !== "codex-oauth") throw new Error("Pending Codex login received an unknown provider");
...
var DshOAuthAccountVault = class {
	...
	async #discoverAccounts() {
		...
		const addCandidate = (candCred, fallbackLabel) => {
			try {
				if (!candCred || typeof candCred !== "object") return;
				const sanitized = sanitizeOAuthCredential(candCred);
				const email = sanitized.email;
				const accountId = sanitized.accountId;
				if (accountId && seenAccountIds.has(accountId)) return;
				if (email && seenEmails.has(email)) return;
				if (accountId) seenAccountIds.add(accountId);
				if (email) seenEmails.add(email);
				const id = this.createId();
				const label = normalizeLabel((email || fallbackLabel || `Account ${discovered.length + 1}`).slice(0, 48));
				discovered.push({
					id,
					label,
		...
		try {
			const raw = await readFile(dshHomePath(".codex-oauth-auth.json"), "utf8");
			const parsed = JSON.parse(raw);
			const cred = parsed?.credential ?? parsed;
			if (cred?.type === "oauth" && typeof cred?.access === "string") addCandidate(cred, "Default Account");
		} catch {}
		try {
			const accountsDir = dshHomePath("accounts");
			...
			for (const file of files) if (file.endsWith(".json")) try {
				...
				const parsed = JSON.parse(raw);
				const cred = parsed?.credential ?? parsed;
				if (cred?.type === "oauth" && typeof cred?.access === "string") addCandidate(cred, file.replace(/\.codex-oauth-auth\.json$/u, "").replace(/\.json$/u, ""));
			} catch {}
		} catch {}
		try {
			for (const key of [
				"llm-pi-ai/openai-codex",
				"llm-pi-ai/codex-mamdouhfces1997",
				"llm-pi-ai/codex-4steps"
			]) {
				const rec = await this.credentials.readRecord(key);
				if (rec?.kind === "grant" && rec.payload?.type === "oauth" && typeof rec.payload?.access === "string") addCandidate(rec.payload, key.replace("llm-pi-ai/", ""));
			}
		} catch {}
		const legacy = await this.#legacyCredential();
		if (legacy?.credential !== void 0) addCandidate(legacy.credential, DEFAULT_LABEL);
		return discovered;
	}
	async #ensurePayload() {
		...
		if (discovered.length === 0) return void 0;
		const activeId = discovered[0].id;
		const legacyAccountId = activeId;
...
function assertProvider(providerId) {
	if (providerId !== PROVIDER$1 && providerId !== "codex-oauth") throw new Error(`Codex credential store does not own provider ${JSON.stringify(providerId)}`);
}
...
function createCodexAutoSearchProvider(options) {
	return Object.freeze({
		...
		available: () => true,
		async search(request, signal) {
			const p = options.resolveModelProvider?.();
			if (p === "openai-codex" || p === "codex-oauth") return options.codex.search(request, signal);
...
const ALIAS_PROVIDER = "codex-oauth";
...
function apply(ctx) {
	...
	const aliasProfile = Object.freeze({
		...profile,
	...
	const profiles = () => {
		...
		if (key !== profileKey) {
			profileKey = key;
			profileSnapshot = /* @__PURE__ */ new Map([[PROVIDER, profile], [ALIAS_PROVIDER, aliasProfile]]);
		}
	...
	ctx.llm.registerAdapter([PROVIDER, ALIAS_PROVIDER], adapter);
	...
	const codexSearch = createCodexSearchProvider({
		...
		resolveModel: () => {
			...
			return request?.provider === PROVIDER || request?.provider === ALIAS_PROVIDER ? request.model : void 0;
		},
	...
	ctx.web.registerSearchProvider(createCodexAutoSearchProvider({
		...
		resolveModelProvider: () => {
			const p = currentAgent()?.session.requestContext?.()?.provider;
			return p === ALIAS_PROVIDER ? PROVIDER : p;
		},
