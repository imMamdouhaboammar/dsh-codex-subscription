  ...
  async #discoverAccounts() {
    ...
    const addCandidate = (candCred, fallbackLabel) => {
      try {
        ...
        const labelCandidate = email || fallbackLabel || `Account ${discovered.length + 1}`
        const label = normalizeLabel(labelCandidate.slice(0, 48))
        discovered.push({ id, label, credential: sanitized })
      } catch {}
    }

    try {
      const defaultAuthFile = dshHomePath('.codex-oauth-auth.json')
      const raw = await readFile(defaultAuthFile, 'utf8')
      const parsed = JSON.parse(raw)
      const cred = parsed?.credential ?? parsed
      if (cred?.type === 'oauth' && typeof cred?.access === 'string') {
        addCandidate(cred, 'Default Account')
      }
    } catch {}
      ...
      for (const file of files) {
        if (file.endsWith('.json')) {
          try {
            ...
            const cred = parsed?.credential ?? parsed
            if (cred?.type === 'oauth' && typeof cred?.access === 'string') {
              const labelGuess = file.replace(/\.codex-oauth-auth\.json$/u, '').replace(/\.json$/u, '')
              addCandidate(cred, labelGuess)
            }
          } catch {}
        }
      }
    ...
    try {
      for (const key of ['llm-pi-ai/openai-codex', 'llm-pi-ai/codex-mamdouhfces1997', 'llm-pi-ai/codex-4steps']) {
        const rec = await this.credentials.readRecord(key)
        if (rec?.kind === 'grant' && rec.payload?.type === 'oauth' && typeof rec.payload?.access === 'string') {
          addCandidate(rec.payload, key.replace('llm-pi-ai/', ''))
        }
      }
    } catch {}

    const legacy = await this.#legacyCredential()
    if (legacy?.credential !== undefined) {
      addCandidate(legacy.credential, DEFAULT_LABEL)
    }
